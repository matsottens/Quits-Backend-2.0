...
export async function analyzeEmailsForUser(userId, scanId) {
  ...
  const batchSize = 2; // Process 2 emails at a time
  let analyzedCount = 0;
  let errorCount = 0;

  for (let i = 0; i < emailData.length; i += batchSize) {
    const batch = emailData.slice(i, i + batchSize);
    console.log(`Processing batch ${Math.floor(i / batchSize) + 1}/${Math.ceil(emailData.length / batchSize)}`);
...
Only return valid JSON. If the email is not subscription-related, set is_subscription to false and all other fields to null.
    ...
    batchResults.forEach(result => {
      if (result.success && result.analyzed) {
        analyzedCount++;
